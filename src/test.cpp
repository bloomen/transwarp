...
TEST_CASE("task_type_to_string") {
    ...
    REQUIRE("value" == transwarp::to_string(transwarp::task_type::value));
...
TEST_CASE("value_task") {
    auto t = make_task(transwarp::value, 42);
    REQUIRE(42 == t->get());
    ...
    auto n = t->get_node();
    REQUIRE(0u == n->get_id());
    REQUIRE(transwarp::task_type::value == n->get_type());
    REQUIRE(!n->get_name());
...
TEST_CASE("value_task_with_name") {
    ...
    auto t = make_task(transwarp::value, name, 42);
    REQUIRE(42 == t->get());
    REQUIRE(42 == t->get_future().get());
    REQUIRE(t->was_scheduled());
    REQUIRE(t->is_ready());
...
TEST_CASE("value_task_in_a_graph") {
    auto t1 = make_task(transwarp::value, 42);
    REQUIRE(42 == t1->get());
    auto t2 = make_task(transwarp::value, 13.3);
    REQUIRE(13.3 == t2->get());
...
TEST_CASE("value_task_with_lvalue_reference") {
    const int x = 42;
    auto t = make_task(transwarp::value, x);
    REQUIRE(x == t->get());
}

TEST_CASE("value_task_with_rvalue_reference") {
    int x = 42;
    auto t = make_task(transwarp::value, std::move(x));
    REQUIRE(42 == t->get());
}
