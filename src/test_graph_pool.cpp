...
TEST_CASE("graph_pool_constructor") {
    tw::graph_pool<int> pool(tw::make_task(tw::root, []{ return 42; }), 3, 100);
    REQUIRE(3 == pool.size());
...
TEST_CASE("graph_pool_constructor_overload") {
    tw::graph_pool<int> pool(tw::make_task(tw::root, []{ return 42; }), 3, 5);
    REQUIRE(3 == pool.size());
    REQUIRE(3 == pool.idle_count());
    REQUIRE(0 == pool.busy_count());
    REQUIRE(3 == pool.minimum_size());
...
TEST_CASE("graph_pool_constructor_throws_for_invalid_minimum") {
    auto lambda = []{ tw::graph_pool<int>{tw::make_task(tw::root, []{ return 42; }), 0, 100}; };
    REQUIRE_THROWS_AS(lambda(), tw::invalid_parameter);
}

TEST_CASE("graph_pool_constructor_throws_for_invalid_minimum_maximum") {
    auto lambda = []{ tw::graph_pool<int>{tw::make_task(tw::root, []{ return 42; }), 3, 2}; };
    REQUIRE_THROWS_AS(lambda(), tw::invalid_parameter);
}

TEST_CASE("graph_pool_next_idle_graph") {
    tw::graph_pool<int> pool(tw::make_task(tw::root, []{ return 42; }), 2, 100);
    REQUIRE(2 == pool.size());
...
TEST_CASE("graph_pool_next_idle_graph_with_nullptr") {
    tw::graph_pool<int> pool(tw::make_task(tw::root, []{ return 42; }), 1, 2);
    REQUIRE(1 == pool.size());
    auto g1 = pool.next_idle_graph();
    REQUIRE(g1);
    REQUIRE(0 == pool.idle_count());
...
TEST_CASE("graph_pool_resize_with_max") {
    tw::graph_pool<int> pool(tw::make_task(tw::root, []{ return 42; }), 2, 5);
    REQUIRE(2 == pool.size());
    pool.resize(6);
    REQUIRE(5 == pool.size());
}
...
TEST_CASE("graph_pool_reclaim") {
    tw::graph_pool<int> pool(tw::make_task(tw::root, []{ return 42; }), 2, 4);
    REQUIRE(2 == pool.size());
    auto g1 = pool.next_idle_graph();
    REQUIRE(g1);
    auto g2 = pool.next_idle_graph();
